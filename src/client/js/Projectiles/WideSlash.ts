...
export default class WideSlash {
    ...
    constructor(owner: number, life: number, damage: number, position: THREE.Vector3, direction: THREE.Vector3, speed: number, color: number = 0xffffff, scale: number = 1) {
        ...
        let url = new URL('../../../../assets/models/wide-slash.glb', import.meta.url);
        ...
        loader.load(url.href, (gltf) => {
            ...
            for (let i = 0; i < gltf.animations.length; i++) {
                this._mixer.clipAction(gltf.animations[i]).setDuration(0.9).play();
            }
        ...
        //move in opposite direction of direction vector
        this.object.position.add(direction.clone().multiplyScalar(-1));
